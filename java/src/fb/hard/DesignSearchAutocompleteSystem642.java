...
public class DesignSearchAutocompleteSystem642 {
    ...
    public DesignSearchAutocompleteSystem642(String[] sentences, int[] times) {
        for (int i = 0; i < times.length; i++) {
            insertsString(sentences[i], times[i]);
        }
    }
    ...
    public List<String> input(char c) {
        ...
        if (c == '#') {
            ...
            for(int i=0;i<Math.min(3 , candidate.size()) ; i++){
                ans.add(candidate.get(i).curString);
            }
            return ans;
        }
        return ans;
    ...
    private void insertsString(String s , int t){
    ...
    private List<TrieNode> search(char c ){
        ...
        if(this.searchRoot==null){return ans;}
        if(!this.searchRoot.children.containsKey(c)){
            this.searchRoot=null;
            return ans;
        }
        this.searchRoot=this.searchRoot.children.get(c);
        getAllCandidate(this.searchRoot , ans);
        return ans;
